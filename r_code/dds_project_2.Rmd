...
plot_ly(x=balanced_attrition_factors_data_with_enginering$survery_results,
        y=balanced_attrition_factors_data_with_enginering$worker_type,
        z=balanced_attrition_factors_data_with_enginering$career_type,
        type="scatter3d", 
        color=balanced_attrition_factors_data_with_enginering$Attrition) %>%
  layout(scene=list(xaxis=list(title="Survery Results"),
                    yaxis=list(title="Worker Type"),
                    zaxis=list(title="Career Type")),
         title="Attrition by Survey Results and Worker/Career Type")
...
# Note: Looks like K = 1 is best value
knn_results_df %>%
  ...
  for(i in 1:iterations)
    {
      ...
      train_indices = sample(seq(1:length(data_for_model$Attrition)), 
...
# drop columns with only one value as they are not adding any value
salary_clean_data <- sallery_raw_data[vapply(attrition_raw_data, function(x) length(unique(x)) > 1, logical(1L))]
